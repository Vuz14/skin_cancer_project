...
def plot_training_metrics(history_data, metric_name, dataset_name, model_name, output_dir):
    """
    Vẽ biểu đồ cho các chỉ số training/validation theo epoch (Chỉ để lại hình và legend)
    """
    ...
    for mode_idx, df in history_data.items():
        if metric_name in df.columns:
            ...
            plt.plot(epochs, values, color=colors[mode_idx - 1],
                     label=mode_labels[mode_idx - 1], linewidth=3, marker='o', markersize=8)

    # Chỉ giữ lại phần chú thích các đường
...
def plot_test_metrics(test_data, metric_name, dataset_name, model_name, output_dir):
    """
    Vẽ biểu đồ đường cho các chỉ số test (Chỉ để lại hình và legend)
    """
    ...
    for i, (mode, value, color) in enumerate(zip(modes, values, plot_colors)):
        plt.scatter(mode, value, color=color, s=200, zorder=5, label=mode_labels[mode - 1], edgecolors='black',
                    linewidths=1.5)
        plt.annotate(f'{value:.4f}', (mode, value), textcoords="offset points",
                     xytext=(0, 15), ha='center', fontsize=18, fontweight='bold')

    plt.xticks(modes, [f'Mode {m}' for m in modes], fontsize=22)

    # Chỉ giữ lại chú thích
    handles, labels = plt.gca().get_legend_handles_labels()
    if handles:
        plt.legend(loc='best', fontsize=22)
...
def main():
    base_path = r'D:\skin_cancer_project\src\evaluate'
    output_base_dir = r'D:\skin_cancer_project\src\evaluate\plots'

    models = {
        'checkpoint_efcnb4': 'Efficienet-B4',
        'checkpoint_conv': 'Convnext',
        'checkpoint_resnet50': 'ResNet50',
        'checkpoint_vit16': 'ViT16',
    }
